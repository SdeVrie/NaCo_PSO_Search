...
def check_target_detection(grid):
    ...
    for agent in grid.agents:
        for i, target in enumerate(grid.targets):
            ...
            if distance < agent.radius + target_radius + 3:
...
def RDPSO(grid, max_steps = 1000, step_size = 2):
    ...
    for steps in range(max_steps):
        ...
        for agent_idx, agent in enumerate(agents):
            ...
            # update Pbest & Gbest
            gbest, gx, gy = RDPSO_util.update_GbestPbest(agent, gx, gy, gbest, goal_list_x, goal_list_y, agents, v_x, v_y, agent_idx, steps)
